...
public class GuiIngame extends Gui
{
    ...
    protected void renderTooltip(ScaledResolution sr, float partialTicks) {
    	if(ModManager.getModByName("HUD").isEnabled() && HUD.hotbar.getValueState().booleanValue()) {
			RenderUtil.drawRect(sr.getScaledWidth() / 2 - 92, sr.getScaledHeight() - 23, sr.getScaledWidth() / 2 + 91, sr.getScaledHeight(), Integer.MIN_VALUE);
		   	if(mc.thePlayer.inventory.currentItem == 0) {
		   		...
		   		RenderUtil.drawRect((sr.getScaledWidth() / 2) - 92 + mc.thePlayer.inventory.currentItem * 20, sr.getScaledHeight() - 23, (sr.getScaledWidth() / 2) + 91 - 20 * (8 - mc.thePlayer.inventory.currentItem), sr.getScaledHeight(), Integer.MAX_VALUE);
		   	}
		   	GlStateManager.disableBlend();
		}
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
            ...
            if(ModManager.getModByName("HUD").isEnabled() && HUD.stuff.getValueState().booleanValue()) {
            	ScaledResolution res = new ScaledResolution(this.mc);
                this.renderStuffStatus(res);
            }
    ...
    private void renderStuffStatus(ScaledResolution scaledRes) {
   		...
   		for (int slot = 3, xOffset = 0; slot >= 0; slot--) {
   			...
   			if (stack != null) {
   				...
   				mc.getRenderItem().renderItemIntoGUI(stack, scaledRes.getScaledWidth() / 2 + 15 - xOffset, scaledRes.getScaledHeight() - 70 - (yOffset / 2) + 15);
   				xOffset -= 18;
   			}
   		}
   	}
