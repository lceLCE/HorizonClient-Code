...
import me.James_.ui.Particle;
import me.James_.ui.altManager.GuiAltManager;
import me.James_.ui.menu.UIMenu;
import me.James_.ui.menu.UIMenuSlot;
import me.James_.ui.partcles.ParticleManager;
import me.James_.ui.partcles.ParticleSnow;
import me.James_.ui.shader.ParticleShader;
...
public class GuiMainMenu extends GuiScreen implements GuiYesNoCallback
{
    ...
    public void initGui() {
        ...
        slots1.add(new UIMenuSlot("AltManager", new GuiAltManager(this)));
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    	...
    	RenderUtil.drawImage(new ResourceLocation("J/bg2.png"), (int)this.animationX, 0, res.getScaledWidth(), res.getScaledHeight());
    	for (Particle particle : this.particles) {
            particle.drawScreen(mouseX, mouseY, res.getScaledHeight());
        }
    	UnicodeFontRenderer font = Client.fontManager.tahoma60;
    	int xStart = (int)this.animationX + (this.width - font.getStringWidth(Client.CLIENT_NAME)) / 2 + 5;
        int yMid = this.height / 2 - 40;
        font = Client.fontManager.tahoma70;
        String s = Client.CLIENT_name + " " + Client.CLIENT_VER;
        font.drawString(s, (float)xStart - 50, (float)yMid - (float)font.FONT_HEIGHT / 1.2f,  RenderUtil.rainbow(100));
        Gui.drawRect((float)xStart, (float)(yMid + 20), (float)xStart, (float)(yMid + 20) + 0.5F, Colors.GREY.c);
        this.menu.draw(mouseX, mouseY);
        if(this.menu.x == 0.0F) {
           this.animationX = RenderUtil.INSTANCE.getAnimationState(this.animationX, 100.0D, (100.0D - this.animationX + 1.0D) * 30.0D);
        } else {
           this.animationX = RenderUtil.INSTANCE.getAnimationState(this.animationX, 0.0D, (Math.abs(this.animationX) + 1.0D) * 30.0D);
        }
    ...
    private void do3dEffects(int mouseX, int mouseY, float partialTicks) {
		...
		{
			GL11.glVertex2d(RANDOM.nextInt(randBig) - divider, RANDOM.nextInt(randBig) - divider);
			GL11.glVertex2d(RANDOM.nextInt(randBig) - divider, RANDOM.nextInt(randBig) - divider);
		}
		GL11.glEnd();
		float f1 = 0.1f;
