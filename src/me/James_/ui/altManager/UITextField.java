...
public class UITextField extends GuiTextField {
   ...
   public void drawOutlinedRect(float x, float y, float width, float height, float lineSize, int lineColor, int backgroundColor) {
   ...
   public void drawTextBox() {
      if(this.getVisible()) {
         if(this.getEnableBackgroundDrawing()) {
            drawOutlinedRect((float)this.xPosition, (float)this.yPosition, (float)(this.xPosition + this.width), (float)(this.yPosition + this.height), 0.7F, this.isFocused?Colors.DARKBLUE.c:Colors.GREY.c, 0);
         }
         ...
         if(s.length() > 0 && flag && j < s.length()) {
            var10000 = (int)this.fontRendererInstance.drawString(s.substring(j), (float)j1, (float)i1, -1, false);
         }
         ...
         if(k != j) {
            ...
            drawRect(k1, i1 - 1, l1 - 1, i1 + 1 + this.fontRendererInstance.FONT_HEIGHT, ClientUtil.INSTANCE.reAlpha(Colors.DARKBLUE.c, 0.3F));
         }
      }
