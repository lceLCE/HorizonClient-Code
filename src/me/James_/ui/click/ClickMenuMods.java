...
import me.James_.ui.click.options.UIMode;
import me.James_.ui.click.options.UISlider;
import me.James_.ui.click.options.UIToggleButton;
    ...
    public void draw(final int mouseX, int mouseY) {
        ...
        for (final Mod m : this.modList) {
            ...
            if (m.hasValues()) {
                ...
                GL11.glTranslated(-xMid, -yMid, 0.0);
                if (arrowHover) {
                    RenderUtil.drawImage(new ResourceLocation("J/icons/arrow-down.png"), this.x + this.width - 8, yAxis + (height - size) / 2 + 1, size, size, new Color(0.7058824f, 0.7058824f, 0.7058824f));
                } else {
                    RenderUtil.drawImage(new ResourceLocation("J/icons/arrow-down.png"), this.x + this.width - 8, yAxis + (height - size) / 2 + 1, size, size, new Color(0,0,0));
                }
                GL11.glPopMatrix();
            }
            ...
            font.drawString(m.getName(), (float)(this.x + (this.width - font.getStringWidth(name)) / 2) - 15, (float)(yAxis + (height - font.FONT_HEIGHT) / 2), m.isEnabled() ? 0 : Colors.GREY.c);
            if (m.openValues) {
