...
public class UISlider {
    ...
    public double draw(final float value, final int mouseX, final int mouseY, final int x, final int y) {
        ...
        font.drawString(strValue, x + this.width / 2 - vWidth / 2.0f + 0.5f, y + this.height + 2.5f, Colors.BLACK.c);
    ...
    private double changeValue(final float value, final int mouseX, final int mouseY, final int x, final int y) {
        ...
        Gui.drawRect((float)x, (float)y, (this.lastMouseX == -1.0f) ? ((float)(int)val) : (x + this.width * this.lastMouseX), (float)(y + this.height), new Color(51,102,205).getRGB());
        Gui.circle((float)val, y + 1, 2f, new Color(51,102,205).getRGB());
        if (this.drag) {
